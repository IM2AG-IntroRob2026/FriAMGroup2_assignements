...
### Default parameters (side_length=2.0, speed=1.0)
```bash
ros2 action send_goal /draw_square turtle_square_interfaces/action/DrawSquare '{side_length: 0.0, speed: 0.0}' --feedback
```
...
> **Note:** Use single quotes `'...'` around the YAML argument, not double quotes.
> Double quotes work in the terminal directly but break in `.sh` scripts.
